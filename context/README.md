...
| `04-lessons.md` | The Lessons tab: browse/search, and the untested practice modes per lesson (Practice/Vocab List/Flashcards/Match/Learn) plus where the ladder sits. Read with `02`. |
| `05-shared-and-audio.md` | Cross-cutting pieces: the `Pronouncer`/`Speech` audio seam, the shared quiz/flashcard/swipe components, `CardPager`, the two independent language settings, and notifications (`StreakReminder` on-device, `PushService`'s dual-SDK token fan-out and the 3.0.2 main-thread lesson). |
